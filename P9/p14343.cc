...
struct Alumne {
    ...
    int num; // Numero que identifica enviaments
};
...
void calcula_resultats(const Historia&v, Resultats&r) {
    int i = 0;
    r.max_temp.num = 0, r.env_verds.num = 0, r.ex_verds.num = 0, r.ex_verms.num = 0, r.ex_in.num = 0;
 
    while (i < v.size()) {
        string alum = v[i].dni;
        int auxenv_verds = 0, auxex_in = 0, auxex_verds = 0, auxex_verms = 0;
        while (i < v.size() and v[i].dni == alum) {
            string exer = v[i].exer;
            bool hihaverd = false, hihagroc = false, hihavermell = false;
